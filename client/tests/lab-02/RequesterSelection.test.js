...
describe("Lab 02 - Dev Requester Context & Selection UI", () => {
    ...
    describe("RequesterSelection Component", () => {
        it("renders selection screen with Zen Green header, Lab 3 notice banner, dropdown, and Continue button", async () => {
            ...
            expect(screen.getByText(/Authentication arrives in Lab 3/i)).toBeInTheDocument();
        ...
        it("displays error message when fetching requesters fails", async () => {
            vi.spyOn(api, "fetchRequesters").mockRejectedValue(new Error("Unable to retrieve requesters"));
            render(_jsx(RequesterSelection, { onSelectRequester: vi.fn() }));
            await waitFor(() => {
                expect(screen.getByTestId("error-message")).toHaveTextContent("Unable to retrieve requesters");
            });
            const continueBtn = screen.getByTestId("continue-btn");
    ...
    describe("Navbar Component", () => {
        it("renders brand logo and active requester name with Change Requester button", () => {
            ...
            render(_jsx(Navbar, { activeRequester: activeUser, onChangeRequester: handleChange }));
            expect(screen.getByText(/TokTickIT/i)).toBeInTheDocument();
            expect(screen.getByTestId("active-requester-name")).toHaveTextContent("Alice Johnson");
