...
describe("Lab 03 Requester Workflow UI Component Tests (TicketDetail)", () => {
  const mockTicket: apiModule.Ticket = {
    id: 12,
  ...
  it("renders 'Problem Appears Resolved' button and updates to badge when clicked", async () => {
    ...
    render(<TicketDetail ticketId={12} userId={1} onBack={() => {}} />);
    ...
    await waitFor(() => {
      ...
      expect(screen.getByTestId("requester-resolved-badge")).toHaveTextContent(
  ...
  it("renders public comments and allows posting a new public comment", async () => {
    ...
    await waitFor(() => {
      expect(apiModule.postPublicComment).toHaveBeenCalledWith(
        12,
        "Here is additional diagnostic log.",
        1
      );
    });
  });
