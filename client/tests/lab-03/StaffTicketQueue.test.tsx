...
describe("Lab 03 IT Staff Ticket Queue UI Component Tests (StaffTicketQueue.test.tsx)", () => {
  const mockTickets: apiModule.Ticket[] = [
    {
      id: 101,
  ...
  const mockCategories: apiModule.Category[] = [
    { id: 1, name: "Hardware" },
    { id: 2, name: "Network" },
  ];

  const mockPaginatedData: apiModule.PaginatedTickets = {
    items: mockTickets,
    meta: {
      currentPage: 1,
      limit: 10,
  ...
  it("TC-UI-QUEUE-01: Renders ticket queue table displaying Ticket No, Summary, Category, Priorities, Status, and Owner", async () => {
    ...
    render(<StaffTicketQueue userId={3} onSelectTicket={onSelectTicket} />);
  ...
  it("TC-UI-QUEUE-02: Typing keyword search updates search input and calls fetchTicketQueue", async () => {
    ...
    render(<StaffTicketQueue userId={3} onSelectTicket={() => {}} />);
    ...
    await waitFor(() => {
      expect(fetchQueueSpy).toHaveBeenCalledWith(
