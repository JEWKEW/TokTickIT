...
*[Figure: Development Requester Selection screen mockup — dropdown to choose a Development Requester, with an "Authentication coming in Lab 3" notice and a Continue button]*
...
*[Figure: My Tickets screen mockup — search box, Category/Requested Priority/IT Priority/Current Status filters, sortable table columns (Ticket No., Created Date, Summary, Category, Requested Priority, IT Priority, Current Status, Ticket Owner, Last Updated), and pagination]*
...
|---|---|---|
| 1. Git Use with Engineering Workflow | 10 | Screenshot evidence that you used the git workflow by showing your commit history in the final main branch showing you created various feature branches that were eventually merged into the staging branch and then the main branch. Evidence you used GitHub Project and final Kanban with all Issues in Done. Rendered reviewer.md with reviewer identity, PR links, comments given and received, responses, and approvals. Include README and .gitignore content evidence. Directory Structure of your repository in your IDE |
...
| 6. Working Ticket Screen: Create Mode | 10 | Readable screenshots of initial, validation failure, submitting, success, API failure, and invalid-attachment states. Demonstrate that the official Ticket Number and saved values come from the backend/database. Include the Development Requester Selection screen, active-user dropdown, selected-user display, Change Requester action, loading state, and failure state. 1. Show that the Requester field is populated from the Development Requester selected before entering the application and that the saved Ticket contains the matching requesterId. 2. Open Create Ticket at a desktop viewport and show reference data loaded from the database. 3. Attempt an invalid submission and show field-level messages. 4. Select one valid and one invalid attachment and explain the result. 5. Stop the backend or simulate failure and show the safe error state with form values preserved. |
...
To make grading faster and more consistent, you must format your PDF using the following format:
