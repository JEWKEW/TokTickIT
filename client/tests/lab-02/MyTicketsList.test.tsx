...
describe("MyTicketsList Component", () => {
  ...
  const mockTicketItems: api.Ticket[] = [
    {
      id: 101,
  ...
  it("toggles sorting order when clicking table header columns", async () => {
    ...
    await waitFor(() => {
      expect(fetchSpy).toHaveBeenLastCalledWith(
        expect.objectContaining({ sort: "summary", order: "asc" }),
        1
      );
    });
  });
  ...
  it("verifies data isolation when switching user IDs", async () => {
    ...
    const { rerender } = render(<MyTicketsList userId={1} />);
    ...
    rerender(<MyTicketsList userId={2} />);
